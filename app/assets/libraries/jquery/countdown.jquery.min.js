    ...
    $.fn.countdown.begin = function (parent, settings) {
        ...
        if (!settings.init) {
            ...
            $.each(timespan, function (k, v) {
                ...
                container = $('<div/>').addClass('2xl:w-1/3 w-1/3 mt-6 text-center text-md').attr('id', k);

                // Create wrapper element
                wrapper = $('<div/>').addClass('wrapper');

                // Create time element
                time = $('<p/>').addClass('time font-bold text-5xl text-465852').text(v < 10 ? '0' + v : v.toLocaleString());

                if (settings.labels) {
                    // Create label element

                    console.log('$.fn.countdown.singularize(k)', defineVN['day'])
                    label = $('<p/>').addClass('label').text((v === 1 ? defineVN[$.fn.countdown.singularize(k)] : defineVN[k]));
            ...
            $.each(timespan, function (k, v) {
                ...
                $('.label', '#' + k).text((v === 1 ? defineVN[$.fn.countdown.singularize(k)] : defineVN[k]));
            });
        }
